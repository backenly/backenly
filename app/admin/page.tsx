          ...
          <div className="fixed right-0 top-0 h-full w-full max-w-xl z-50 bg-[#0c0c0e] border-l border-white/[0.08] overflow-y-auto flex flex-col shadow-2xl">
            ...
            {detail && !loadingDetail && (
              <div className="flex-1 overflow-y-auto">
                ...
                {/* AI Conversations — every prompt this user typed + Backenly AI's answer */}
                <div className="px-6 py-5 border-b border-white/[0.06]">
                  <button
                  ...
                  >
                    ...
                    <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest group-hover:text-zinc-300 transition-colors">AI Conversations</h3>
                    {convos && (
                  ...
                  {convosOpen && (
                    <div className="mt-3">
                      ...
                      {convos && !convosLoading && convos.projects.length === 0 && (
                        <p className="text-zinc-600 text-xs py-2">No AI chat history for this user yet.</p>
                      )}
                                        ...
                                        {p.messages.map(m => (
                                          <div key={m.id} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'}`}>
                                            <div className="flex items-center gap-1.5 mb-1 px-0.5">
                                              <span className={`text-[9px] font-bold uppercase tracking-wider ${m.role === 'user' ? 'text-violet-300' : 'text-zinc-400'}`}>
                                                {m.role === 'user' ? 'User' : 'Backenly AI'}
                                              </span>
