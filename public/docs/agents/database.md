...
`apply_migration { sql }` takes `CREATE TABLE`, `ALTER TABLE` (`ADD COLUMN`, `RENAME COLUMN`, `ADD CONSTRAINT`, `ALTER COLUMN SET`/`DROP NOT NULL`, `SET`/`DROP DEFAULT`) and `CREATE [UNIQUE] INDEX`. Each statement becomes a governed action. Every statement is checked before any runs: one it cannot map is refused, with the tool to use instead, and nothing is applied. It is not one transaction: a statement that fails while running stops the migration, and the response lists the statements in `applied` (which did take effect; do not replay them) and in `remaining`. Dropping a table or column, or truncating one, goes through `backend_chat` and waits for a human's approval.
