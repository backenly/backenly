 ...
 *   parent → worker : { type:'validate', code, id }
 *   worker → parent : { type:'validated', id, error }             (error null = valid)
 */
...
// The checks runFunction makes before it runs anything, a check that every
// ctx.require() names a package this worker provides, and a compile of the same
// wrapper. Nothing else: no ctx or event exists, and no statement of the
// function executes, because compiling a function expression does not call it.
// Deploying agent-written code (functions deploy_code) asks this, so code that
// could never run here is refused before it is stored, by the file that runs it.
function validateFunction(id, code) {
  const reply = (error) => parentPort.postMessage({ type: 'validated', id, error })
  if (typeof code !== 'string' || !code.trim()) return reply('The function body is empty.')
  const processedCode = preprocessImports(code)
  const blocked = checkBlockedPatterns(processedCode)
  if (blocked) return reply(blocked)
  for (const m of processedCode.matchAll(/\bctx\s*\.\s*require\s*\(\s*['"]([^'"]+)['"]\s*\)/g)) {
    if (!WHITELISTED_PACKAGES.has(m[1])) {
      return reply(`ctx.require('${m[1]}') is not available here. Available packages: ${[...WHITELISTED_PACKAGES].join(', ')}.`)
    }
  }
  try {
    new vm.Script(`(async function handler(event, ctx) {\n${processedCode}\n})`, { filename: 'ai-function.js' })
  } catch (err) {
    return reply(`Syntax error: ${err.message}`)
  }
  return reply(null)
}

// ─── Entry: wait for 'run' or 'validate' message ─────────────────────────────
parentPort.on('message', (msg) => {
  if (msg.type === 'validate') {
    validateFunction(msg.id, msg.code)
    return
  }
  if (msg.type === 'run') {
