...
- Autonomy cadence by plan: EVERY plan runs the loop every minute, Free included. Plans differ on how much it may fix, not how often it looks: Free = up to 5 fixes per window and ~120 windows/month then detection-only, Pro = 20 fixes per window and unlimited windows, Enterprise = custom. The loop is DETERMINISTIC end to end — probes detect drift, and each finding maps to a typed action that compiles to SQL. It runs no model, so it cannot draw AI credits. Say autonomy is included and uses no AI credits because it uses no AI. Never say it is company-funded.
