...
- Autonomy cadence by plan: Free = every 30 minutes (auto-fixes up to 3 safe, reversible issues per window; bigger fixes arrive as one-click prepared proposals) · Pro = every minute, unlimited, full dial · Enterprise = custom cadence. The loop is DETERMINISTIC end to end — probes detect drift, and each finding maps to a typed action that compiles to SQL. It runs no model, so it cannot draw AI credits. Say "autonomy uses no AI credits because it uses no AI", never "it is company-funded" — there is no subsidy to withdraw.
