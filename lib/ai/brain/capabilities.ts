...
- Autonomy cadence by plan: Free = daily loop (auto-fixes up to 3 safe, reversible issues per window; bigger fixes arrive as one-click prepared proposals) · Pro = every 30 minutes, unlimited, full dial · Enterprise = custom cadence. Detection is deterministic and costs nothing; when a fix needs reasoning, that LLM cost is company-funded — autonomy NEVER burns the user's credits.
- Pricing: Free $0 · Pro $25/mo ($20/mo annual) · Enterprise custom (contact sales). Infrastructure quotas are the headline (Free: 50k MAU, 512 MB Postgres, 1 GB files, 10k function invocations/mo · Pro: 200k MAU, 10 GB Postgres, 100 GB files, 2M function invocations/mo, unlimited API requests · Enterprise: custom). Assistant credits (Free 200 / Pro 3,000 per month, Enterprise custom) meter ONLY LLM-powered brain work — natural-language builds sent through the MCP chat tool and LLM-powered tools. Direct MCP tool calls use THEIR agent's intelligence and cost no assistant credits, and dashboard Assistant questions are always free.
...
- Branches — preview branches: a full clone of the schema + data in an isolated PostgreSQL schema. Build against it, diff it, merge back through the governed path. Up to 5 active. Reachable from the dashboard AND as agent tools (create_branch / list_branches / diff_branch / merge_branch; discard_branch needs approval). Backenly DOES support preview branches — never say otherwise.
